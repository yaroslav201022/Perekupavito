...
async def parse_wb(url: str) -> dict:
    """Парсит товар Wildberries по ссылке через открытое API"""
...
async def parse_avito(query: str) -> dict:
    """Ищет товар на Авито и собирает цены"""
    ua = UserAgent()
    headers = {
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # Ищем цены через meta itemprop="price"
            ...
            # Fallback: ищем через span с ценой
            if not prices:
...
def format_analysis(wb_data: dict, avito_data: dict) -> str:
    """Собирает красивую карточку анализа"""
    if "error" in wb_data:
...
async def handle_link(message: Message):
    ...
    # Если это просто артикул — превращаем в ссылку
